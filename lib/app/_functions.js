...
var view = {
	...
	markdown: function(body)
	{
		...
		const bodyWithPlantuml = body.replace(new RegExp('@startuml((.|\n)*)@enduml', 'gim'), this.plantuml)
		console.log('bodyWithPlantuml', bodyWithPlantuml)
		return md.render(bodyWithPlantuml);
	},

	plantuml: function(content) {
		const encodedContent = unescape(encodeURIComponent(content));
		return `![Plant UML diagram](http://www.plantuml.com/plantuml/img/${encode64(zip_deflate(encodedContent, 9))})`;
